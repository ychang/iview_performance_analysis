...
class ImageProcess:
    ...
    def sort_current_image(self, sort_category):

        temp_output_dir = self.output_dir + 'category_' + sort_category

        Path(temp_output_dir).mkdir(parents=True, exist_ok=True)

        #copyfile(self.file)        
        base_filename = os.path.basename(self.file)

        print('Copy file : {0}'.format(self.file+'.jpg'))

        copyfile(self.file+'.jpg', temp_output_dir+'\\'+base_filename+'.jpg')
        copyfile(self.file+'.txt', temp_output_dir+'\\'+base_filename+'.txt')
        #print('Save file '+os.path.basename(self.file))
        #plt.savefig(self.output_dir+os.path.basename(self.file))
...
def draw_image(filename):
    ...
    for bbox in bboxes:
        ...
        ax.add_patch(Rectangle((x_start, y_start), width, height,  linewidth=2, edgecolor=color, fill = False))
        #ax.text(x_center, y_center, '{0}'.format(bbox['cs']), color = text_color)
...
def close_figure(event):
    ...
    if event.key >= '1' and event.key <= '9':

        imgprc.sort_current_image(event.key)
