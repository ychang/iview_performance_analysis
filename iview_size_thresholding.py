...
titleBox('IVIEW+ (HDS-K01/K02) 학습데이터 TWEAKER')

parser = argparse.ArgumentParser(description='IVIEW+ (HDS-K01/K02) 학습데이터 TWEAKER')

parser.add_argument('-i', '--input', type = str, help='Inference Data Directory', required = True)
parser.add_argument('--head', type = float, help='Size threshold for Head', required = False)
parser.add_argument('--upperbody', type = float, help='Size threshold for Upperbody', required = False)
parser.add_argument('--person', type = float, help='Size threshold for Person', required = False)
...
thresholding_head = args.head
thresholding_upperbody = args.upperbody
thresholding_person = args.person

print ('Input Dir = {0}'.format(input_dir))
print ('Output Dir = {0}'.format(output_dir))

print ('Thresholding HEAD = {0}'.format(thresholding_head))
print ('Thresholding UPPERBODY = {0}'.format(thresholding_upperbody))
print ('Thresholding PERSON = {0}'.format(thresholding_person))
...
for filename in files:
    ...
    with open(filename,'r') as file:
        ...
        for line in file:
            ...
            if thresholding_head is not None and bbox_info['class'] == 0:
                rewrite = size < thresholding_head

            if thresholding_upperbody is not None and bbox_info['class'] == 1:
                rewrite = size < thresholding_upperbody

            if thresholding_person is not None and bbox_info['class'] == 2:
                rewrite = size < thresholding_person

            if rewrite:
                #print('Filename {0} Class {1} size {2}is less then threshold.'.format(filename, bbox_info['class'], size))
                new_file = True
            else:
                new_lines.append(line)


        if new_file and len(new_lines) >0 :                

            Path(output_dir).mkdir(parents=True, exist_ok=True)

            output_filename = output_dir+os.path.basename(filename)

            print('File {0} has been rewritten.'.format(os.path.basename(filename)))
